...
// Bots are capped on their own rather than at MAX_PLAYERS - 1. The table got bigger;
// the number of bots a host may ask for deliberately did not, because a table that is
// mostly bots is not the game anybody came for.
const MAX_BOTS = 5;
...
// Why this Remove Card may not be played, or null if it may.
//
// Card indices arrive from a client, so every one of these is a rule about a number
// somebody else chose: the wrong index takes the wrong card, and an index past the end
// takes `undefined` and puts it in the discard pile as a card that does not exist.
const removeCardRefusal = (target, cardIndex, isSpecial) => {
  // Only an active hand. A stood or busted player's round is over and their cards are
  // the record of it.
  if (target.status !== 'active') return 'You can only remove cards from active players.';

  const cards = isSpecial ? target.specialCards : target.regularCards;
  ...
  if (isSpecial && cards[cardIndex] === 'RC') return 'You cannot remove a Remove Card.';

  return null;
};

// Why this Steal may not be played, or null if it may.
const stealCardRefusal = (player, target, cardIndex, isSpecial) => {
  if (target.id === player.id) return 'You cannot steal from yourself.';
  // Unlike Remove Card, a stood player is fair game - their cards still score.
  if (target.status === 'busted') return 'You cannot steal from busted players.';
